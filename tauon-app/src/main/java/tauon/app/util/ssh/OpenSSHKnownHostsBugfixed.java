 ...
 */
public class OpenSSHKnownHostsBugfixed
    ...
    public OpenSSHKnownHostsBugfixed(Reader reader) throws IOException {
        this(reader, LoggerFactory.DEFAULT);
    }


    public OpenSSHKnownHostsBugfixed(File khFile)
            throws IOException {
        this(khFile, LoggerFactory.DEFAULT);
    }

    public OpenSSHKnownHostsBugfixed(File khFile, LoggerFactory loggerFactory)
            throws IOException {
        this.khFile = khFile;
    ...
    public OpenSSHKnownHostsBugfixed(Reader reader, LoggerFactory loggerFactory) throws IOException {
        this.khFile = null;
        log = loggerFactory.getLogger(getClass());
    ...
    public class EntryFactory {
        ...
        public KnownHostEntry parseEntry(String line)
            ...
            if (type != KeyType.UNKNOWN) {
                ...
                Buffer.PlainBuffer buffer = new Buffer.PlainBuffer(keyBytes);
                key = buffer.readPublicKey();
    ...
    public static class HostEntry implements KnownHostEntry {
        ...
        final OpenSSHKnownHostsBugfixed.Marker marker;
        ...
        public boolean verify(PublicKey key) throws IOException {
            if (marker == Marker.CA_CERT && key instanceof Certificate<?>) {
                byte[] key2 = ((Certificate<?>) key).getSignatureKey();
                    byte[] keyBytes = Base64.getDecoder().decode(key2);
                Buffer.PlainBuffer buffer = new Buffer.PlainBuffer(keyBytes);
                    final PublicKey caKey =  buffer.readPublicKey();
        ...
        private String getKeyString(PublicKey pk) {
            ...
            type.putPubKeyIntoBuffer(pk, buffer);
            return Base64.getEncoder().encodeToString(buffer.getCompactData());
        }
